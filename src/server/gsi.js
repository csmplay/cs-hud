...
export const registerGsiRoutes = (router, websocket) => {
  router.post("/gsi", (context) => {
    ...
    if (!userAgent.startsWith("Valve/Steam HTTP Client"))
  ...
  router.get("/gsi", (context) => {
    context.type = "application/json";
    context.body = gsiState;
...
const updateLastKnownPlayerObserverSlot = (body) => {
  ...
  for (const [steam64Id, player] of Object.entries(body.allplayers)) {
    if (player.observer_slot === null || player.observer_slot === undefined)
      continue;
    additionalState.lastKnownPlayerObserverSlot[steam64Id] =
...
const updateRoundDamages = (body, mapChanged) => {
  ...
  for (const [steam64Id, player] of Object.entries(body.allplayers || {})) {
    ...
    if (
      ...
      additionalState.roundDamages[steam64Id][roundNumber] =
